Shader "Custom/WaterShader" 
{
	Properties 
	{
		...
		_waterPeriod ("Period", Range(0.1,20)) = 6
		_waterMagnitude ("Magnitude", Range(0,0.05)) = 0.0215
		_offset ("offset", Range(0,10)) = 1.65
...
//				o.position_in_world_space = mul(_Object2World, v.vertex);
			...
			float2 sinusoid (float2 x, float2 m, float2 M, float2 periodo) {
				...
				return M * (1.0 + sin(x * coefficiente)) + m;
			}
			...
			fixed4 frag (v2f_vct i) : COLOR
			{
				...
				float2 m = float2(_waterMagnitude, _waterMagnitude);
				if(_reverse == 1)
				{
					m.y *= -1;
				}
				...
				i.grabUV.xy += waterDisplacement.xy;
